...
- TUM RGB-D `freiburg1_xyz` and ARKitScenes `42444511` both passed the known-pose RGB-D lane mechanically, but both were rejected after visual review. Freiburg1 produced a worthless gallery scene, and ARKitScenes was nearly static with too little parallax. Their generated gallery jobs and local dataset copies were deleted on 2026-07-01.
- The official Graphdeco/Inria `T&T+DB COLMAP` archive was added as a direct `colmap_dataset` benchmark source on 2026-07-01. The first active scene is `graphdeco-tandt-truck-colmap-reference`; `train`, `playroom` and `drjohnson` are also present in the manifest for controlled follow-up runs. These scenes are technical benchmarks only until dataset/media rights are reviewed for public or commercial use.
