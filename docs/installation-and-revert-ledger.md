...
Result: pass. `apt-get -s install ...` reported `148` new packages, `0` upgraded, `0` removed, and `27` not upgraded before the initial manual install. After that manual install, the first CMake attempt reached CUDA configuration and detected the Blackwell/sm_100 PTX workaround, then failed because OpenImageIO referenced missing `/usr/include/opencv4` and COLMAP CUDA targets could not find `CUDA::curand`. A follow-up dry-run for `libopencv-dev libcurand-dev-12-8` reported `95` new packages, `0` upgraded, `0` removed, and `27` not upgraded; the user installed those packages manually. The sidecar build then passed and installed `outputs/tools/colmap-cuda/bin/colmap`, which reports `COLMAP 4.0.4 ... with CUDA`.
Validation result: pass. `/usr/bin/colmap` CPU fallback still passes `scripts/validate-colmap-binary.py`. The sidecar passes CPU and GPU validation with `--allow-gpu --qt-offscreen`; GPU logs show SIFT feature extraction and matching on GPU device `0`.
Notes: Current inventory before installation found `git`, `g++`, `build-essential`, `libssl-dev` and `/usr/local/cuda-12.8/bin/nvcc` present; `cmake` and `ninja` missing from PATH. Docker is not installed. `libcurand-dev-12-8` depends on `libcurand-12-8` and does not depend on NVIDIA driver packages. COLMAP 4.0 uses `FeatureExtraction.*` / `FeatureMatching.*` option names where Ubuntu COLMAP 3.9 uses `SiftExtraction.*` / `SiftMatching.*`, so `scripts/lab-pipeline.py` and `scripts/validate-colmap-binary.py` now choose option names from command help.
...
Revert plan: delete ignored local artifacts `data/datasets/tumrgbd/`, `data/datasets/arkitscenes/`, and generated jobs matching `outputs/jobs/tumrgbd-freiburg1-xyz-reference-*`; revert tracked manifest/docs/script changes if abandoning these lanes.
Result: pass for the lane, reject for the ARKit sample. TUM produced `62249` packaged splats from `300` frames in `splatfacto_preview`. ARKitScenes VGA produced `790806` packaged splats from `600` frames in `splatfacto_reference`, but the source capture was too static and close to flat geometry to be useful.
Notes: The generated ARKitScenes `arkitscenes-42444511-reference-*` jobs were deleted on 2026-07-01 after visual review. Keep the converter for future ARKit/known-pose tests, but do not use ARKitScenes `42444511` as a reference sample.
