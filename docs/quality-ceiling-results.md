...
| Job | Profile | Method | Images | Viewer splats | Viewer PLY | Splat stage time | Eval result | Notes |
| --- | --- | --- | ---: | ---: | ---: | ---: | --- | --- |
| `graphdeco-tandt-truck-colmap-reference-20260701T115428Z` | `splatfacto_reference` | Splatfacto | 251 | 516331 | 122 MB | 367.6 s total stage, 305.4 s train | PSNR `25.5914`, SSIM `0.8805`, LPIPS `0.1060` | First run passed training, packaging, viewer validation and visual review. Render-review sheet shows close target/render agreement across held-out views. Full export before viewer filtering was `128 MB`. |
| `graphdeco-tandt-truck-colmap-reference-20260701T124842Z` | `splatfacto_ceiling` | Splatfacto-big | 251 | 1370076 | 324 MB | 837.0 s total stage | PSNR `25.7287`, SSIM `0.8849`, LPIPS `0.1410` | Larger full-resolution/downscale-1 ceiling run. PSNR/SSIM improved slightly, but LPIPS regressed and eval FPS fell to `1.12`. Because the archive images are smaller than the COLMAP camera model, this path partly trains on normalized/upscaled inputs rather than extra captured detail. Full export before viewer filtering was `375 MB`. |
| `graphdeco-tandt-truck-colmap-reference-20260701T130322Z` | `splatfacto_big_quality` | Splatfacto-big | 251 | 1067465 | 253 MB | 591.6 s train stage | PSNR `25.7159`, SSIM `0.8865`, LPIPS `0.0908` | Best measured Truck result so far. It improves SSIM and LPIPS over the reference run while keeping eval FPS near the lighter reference path. Full export before viewer filtering was `315 MB`. |

Practical conclusion: this is a much better “known good” reference than the rejected RGB-D samples. It shows that our direct `colmap_dataset` lane can produce a recognizable, navigable 3DGS scene from benchmark-grade inputs quickly. On this scene, `splatfacto_big_quality` is the best default quality choice. The heavier `splatfacto_ceiling` run produces more splats but does not look meaningfully better and is worse on LPIPS. For a stronger visual demo, test a richer scene next, such as `train` or one of the Deep Blending interiors, rather than only pushing this Truck input harder.
